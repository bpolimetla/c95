...
/*  gotoxy(10,23);printf("Press Spacebar for change option");
  ...
  case 13 :{
	   ...
	   {
	    case 1 : gotoxy(20,21);
		     printf("PUSH start=%d &start=%d *start=%d",start,&start,*start);
		     start=push(start);
		     break;
	    case 2 : gotoxy(20,23);printf("POP start=%d &start=%d *start=%d",start,&start,*start);
 ...
 stack *push(stack *tp)
 {
  gotoxy(20,22);printf("PUSH tp=%d &tp=%d *tp=%d",tp,&tp,*tp);
 ...
 stack *pop(stack *tp)
 {
   gotoxy(20,24);printf("POP tp=%d &tp=%d *tp=%d tp->data=%d",tp,&tp,*tp,tp->data);
  if (tp->data == 9999)
     {
     ...
     {
      gotoxy(10,15); printf("Deleted element is %d ",tp->data);
      temp1=tp->next;
      /* tp=tp->next; */
      free(tp);      return(temp1);
      }
 }